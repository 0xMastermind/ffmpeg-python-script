...
def validate_video_codec(video_codec):
    if video_codec not in ["hevc_amf", "libx264"]:
        raise ValueError("视频编码参数无效，仅支持 libx264 和 libx265")
...
def build_ffmpeg_command(file, output_file_name, escaped_font_file, font_color, font_size, video_codec, video_quality, bitrate):
    ...
    if bitrate:
        crf_index = ffmpeg_command.index("-crf")
        if crf_index >= 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video_codec', type=str, default='hevc_amf', help='视频编码器 (hevc_amf, libx264)')
